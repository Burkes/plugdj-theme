...
(function () {
  ...
  function run() {
    ...
    for (var i in requireModules) {
      ...
      if (
        module.prototype &&
        isFunction(module.prototype.onFavorite) &&
        isFunction(module.prototype.onFriends)
      ) {
      ...
      if (
        module.prototype &&
        isFunction(module.prototype.update) &&
        isFunction(module.prototype.vote)
      ) {
      ...
      if (
        module.prototype &&
        isFunction(module.prototype.render) &&
        module.prototype.id === "chat"
      ) {
      ...
      if (
        module instanceof window.Backbone.Model &&
        "fx" in module.attributes
      ) {
        window.plugModules.set(module, "models/currentRoom");
      }

      if (
        module instanceof window.Backbone.Collection &&
        isFunction(module.getAudience)
      ) {
      ...
      if (
        ...
        "avatarID" in module.prototype.defaults &&
        "role" in module.prototype.defaults
      ) {
    ...
    if (chatFacade) {
      ...
      chatFacade.onChatReceived = function newChatFacadeOnChatReceived(
        eventData,
        internal,
        muted,
      ) {
        ...
        var mentionRegex =
          /@(everyone|hosts|managers|bouncers|rdjs|staff|djs|guests)/gi;
        var urlRegex =
          /\b(?:(?:https?:\/\/|www\d{0,3}\.|(?:[a-z0-9](?:[-a-z0-9]*[a-z0-9])?\.)+[a-z]{2,}\/)(?:[^\s()<>]|\([^()]*\))+)/gi;
        var imgRegex =
          /(\b(https?):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|]\.(?:jpg|gif|png)\b)/gi;
        var whitelistRegex =
          /(((blog|tech|support|status|stg)\.plug\.dj)|plug\.dj\/(\@\/|\!\/|\_\/|dashboard|about|founders|team|ba|plot|partners|merch|press|jobs|(purchase|giftsub)(\/[0-9]+)?|subscribe|terms|privacy|legal|(mobile|site)(features|bugs)|(android|ios)\-((landing|force|events)\-page|site\-alert|launchrock\-list|all\-users|fb|twitter|instagram|reddit|blog|ba|left\-nav|(index|community)\-footer|events-page))($|\/))/;

        if (
          !eventData.uid ||
          (!this.mutes[eventData.uid] && !ignores.get(eventData.uid)) ||
          muted
        ) {
          ...
          if (window.jumbomojiEnabled) {
            ...
            var messageWithColons = emoji.replace_unified(
              emoji.replace_colons(
                emoji.replace_emoticons_with_colons(eventData.message),
                true,
                false,
                false,
              ),
          ...
          if (sender) {
            ...
            if (
              sender.hasPermission(User.MANAGER) &&
              mentionRegex.test(eventData.message)
            ) {
              ...
              if (Array.isArray(mention)) {
                for (var length = mention.length, i = 0; i < length; i++) {
                  ...
                  if (
                    (group === "@everyone" &&
                      (role > User.MANAGER ||
                        sender.get("gRole") >= User.MANAGER)) ||
                    (group === "@hosts" &&
                      (role === User.COHOST || role === User.HOST)) ||
                    (group === "@managers" && role === User.MANAGER) ||
                    (group === "@bouncers" && role === User.BOUNCER) ||
                    (group === "@rdjs" && role === User.DJ) ||
                    (group === "@staff" && role > User.DJ) ||
                    (group === "@djs" &&
                      (waitlist.isTheUserPlaying ||
                        waitlist.isTheUserWaiting)) ||
                    (group === "@guests" &&
                      (role > User.MANAGER ||
                        sender.get("gRole") >= User.MANAGER) &&
                      currentUser.get("guest"))
                  ) {
          ...
          var roomMentionRegex = new RegExp(
            "(?!@)(" +
              users
                .map((u) => RegExp.escape(u.get("username")))
                .sort((a, b) =>
          ...
          if (match && Array.isArray(match)) {
            for (var length = match.length, i = 0; i < length; i++) {
              var user = users.find(
                (user) => user.get("username") === match[i],
              );

              if (user) {
                eventData.message = eventData.message.replace(
                  new RegExp("@" + RegExp.escape(match[i])),
                  "<@" + user.get("id") + ">",
                );
              }
            }
          ...
          eventData.timestamp = _$.getChatTimestamp(
            24 === settings.settings.chatTimestamps,
          );
          ...
          var parseLink = function Util(message, messageObject, internal) {
            ...
            if (isObject(messageObject) && !skip) {
              ...
              message = message.replace(imgRegex, function (link) {
                ...
                if (
                  positionOfChunk > 0 &&
                  " " == message.charAt(positionOfChunk - 1)
                ) {
                  beforeLink = "<br/>";
                }
                ...
                loadingClass =
                  (messageObject.uid ? messageObject.uid : 0) +
                  "-" +
                  (~~(2147483646 * Math.random())).toString(16);
                messageObject.images[loadingClass] = link;

                return (
                  '<span class="iph iph-' +
                  loadingClass +
                  (beforeLink ? " break-before" : "") +
                  (afterLink ? " break-after" : "") +
                  '">' +
                  beforeLink +
            ...
            if (!internal) {
              message = message.replace(urlRegex, function (link) {
                link = link.replace(/[`!()\[\]{};:'".,<>?«»“”‘’]+$/, "");
                var target =
                  whitelistRegex.test(link) || link.indexOf("plug.dj") === -1
                    ? "_blank"
                    : "_self";
          ...
          if (["user-action", "moderation"].indexOf(eventData.type) === -1) {
            ...
            eventData.message = emoji.replace_unified(
              emoji.replace_colons(
                emoji.replace_emoticons_with_colons(eventData.message),
                false,
                false,
    ...
    if (chatView) {
      ...
      chatView.prototype.onReceived = function newChatViewOnReceived(
        eventData,
      ) {
        try {
          ...
          var matches = eventData.message.match(
            new RegExp(
              "(?!<@)(everyone|hosts|managers|bouncers|rdjs|staff|djs|guests|" +
                users.map((user) => user.get("id")).join("|") +
                ")(?=>)",
              "g",
            ),
          );
          ...
          if (Array.isArray(matches)) {
            for (var length = matches.length, i = 0; i < length; i++) {
              ...
              var mentionGroupsRegex =
                /everyone|hosts|managers|bouncers|rdjs|staff|djs|guests/gi;

              var user = !mentionGroupsRegex.test(mention)
                ? users.get(mention)
                : void 0;
              var name = "@";
              ...
              eventData.message = eventData.message
                .split("<@" + matches[i] + ">")
                .join('<span class="' + mentionClass + '">' + name + "</span>");
            }
          }
          ...
          if (eventData.un) {
            ...
            if (eventData.uid) {
              ...
              if (
                ...
                ((/emote/i.test(this.lastType) &&
                  /emote/i.test(eventData.type)) ||
                  (!/emote/i.test(this.lastType) &&
                    !/emote/i.test(eventData.type)))
              ) {
                merged = true;
                this.lastText.closest(".msg").append(
                  (this.lastText = window
                    .$("<div/>")
                    .addClass("text")
                    .addClass("cid-" + eventData.cid)
                ...
                if (
                  sender &&
                  sender.get("badge") &&
                  sender.get("badge") !== "blankb"
                ) {
                  $badgeBox.append(
                    window
                      .$("<i/>")
                      .addClass(
                        "bdg bdg-" + util.badgeClass(sender.get("badge")),
                      ),
                  );
                } else {
              ...
              $badgeBox.append(
                window.$("<i/>").addClass("icon icon-" + eventData.icon),
              );
            } else if (
              !("user-action" !== eventData.type && "promo" !== eventData.type)
            ) {
            ...
            if (!merged) {
              ...
              if (clickable) {
                $badgeBox
                  .addClass("clickable")
                  .data("uid", eventData.uid)
                  .on("click", this.fromClickBind);
              ...
              if (eventData.uid && eventData.cid) {
                if (canModChat) {
                  $element
                    .addClass("deletable")
                    .on("mouseenter", this.chatOverBind)
                    .on("mouseleave", this.chatOutBind)
                    .append(
                      window
                        .$("<div/>")
                        .addClass("delete-button")
                        .on("click", this.chatDeleteClickBind)
            ...
            $badgeBox = window
              .$("<div/>")
              .addClass("badge-box")
              .append('<i class="icon icon-plug-dj"></i>');
            $from.append(
              window.$("<span/>").addClass("un").text(Lang.alerts.systemAlert),
            );
            $timestamp = window
              .$("<span/>")
              .addClass("timestamp")
          ...
          if (merged) {
            $ =
              ...
              this.$chatMessages[0].scrollHeight -
                this.$chatMessages.height() -
                this.lastText.height();
          } else {
            $ =
              this.$chatMessages.scrollTop() >
              this.$chatMessages[0].scrollHeight -
      ...
      Events._events["chat:receive"][1].callback =
        chatView.prototype.onReceived;
    }
  }
